...
export const materializeCounterPseudoContent = (
  ...
  host.innerHTML = `<style>${css}</style><section id="ahafair">${html}</section>`;
  document.body.appendChild(host);

  try {
    const root = host.querySelector("#ahafair");
    if (!root) return html;
    ...
    const processPseudo = (
      ...
      span.setAttribute("data-ahafair-counter-generated", pseudo);
