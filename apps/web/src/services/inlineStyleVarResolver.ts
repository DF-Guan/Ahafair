...
export const resolveInlineStyleVariablesForCopy = (html: string): string => {
  ...
  // 临时移除 id="ahafair"，阻断预览区暗色 <style> 通过 #ahafair 选择器匹配到离屏容器
  const ahafairRoot = host.querySelector<HTMLElement>("#ahafair");
  if (ahafairRoot) ahafairRoot.removeAttribute("id");
  ...
  try {
    ...
    if (ahafairRoot) ahafairRoot.setAttribute("id", "ahafair");
