...
describe("wechatCounterCompat", () => {
  it("extracts counter pseudo rules from css", () => {
    const css = `
      #ahafair h2::before { content: 'Part' counter(counterh1); }
      #ahafair h3:before { content: counters(counterh2, "."); }
      #ahafair h4::before { content: "No Counter"; }
    `;
  ...
  it("strips only pseudo rules that contain counter content", () => {
    const css = `
      #ahafair h2::before { content: 'Part' counter(counterh1); color: #333; }
      #ahafair h2 { color: #333; }
      #ahafair h3::before { content: "纯文本"; color: #666; }
    `;

    const stripped = stripCounterPseudoRules(css);
    expect(stripped).not.toContain("counter(counterh1)");
    expect(stripped).toContain("#ahafair h2 { color: #333; }");
  ...
  it("materializes pseudo counter text into real nodes before copy", () => {
    ...
    const getComputedStyleSpy = vi
      ...
      .mockImplementation((elt: Element, pseudoElt?: string | null) => {
        ...
        if (!pseudoElt && tag === "section" && id === "ahafair") {
    ...
    expect(result.match(/data-ahafair-counter-generated="before"/g)?.length).toBe(
      5,
    );
  ...
  it("keeps only final increment/reset declaration from computed style", () => {
    ...
    const css = `
      #ahafair h2::before { content: "Part" counter(counterh1); counter-increment: counterh1 999; }
      #ahafair h2::before { content: "Part" counter(counterh1); counter-increment: counterh1; }
    `;
  ...
  it("supports counters(name, separator) multi-level output", () => {
    ...
    `;
    const css = `
      #ahafair h2::before { content: counter(sec) " "; counter-increment: sec; }
      #ahafair h3::before { content: counters(sub, ".") " "; counter-increment: sub; }
    `;

    const result = materializeCounterPseudoContent(html, css);
    expect(result).toContain(
      '<span data-ahafair-counter-generated="before">1 </span>章节',
    );
    expect(result).toContain(
      '<span data-ahafair-counter-generated="before">1 </span>一级',
    );
    expect(result).toContain(
      '<span data-ahafair-counter-generated="before">1.1 </span>二级',
    );
  ...
  it("supports counters separator containing comma", () => {
    ...
    const css = `
      #ahafair h2::before { content: counter(sec) " "; counter-increment: sec; }
      #ahafair h3::before { content: counters(sec, ", ") " "; counter-increment: sec; }
    `;

    const result = materializeCounterPseudoContent(html, css);
    expect(result).toContain(
      '<span data-ahafair-counter-generated="before">1, 1 </span>子级',
    );
  ...
  it("supports counter styles in counter() and counters()", () => {
    ...
    `;
    const css = `
      #ahafair h2::before { content: counter(part, upper-roman) " "; counter-increment: part; }
      #ahafair h3::before { content: counters(item, "-", lower-alpha) " "; counter-increment: item; }
    `;

    const result = materializeCounterPseudoContent(html, css);
    expect(result).toContain(
      '<span data-ahafair-counter-generated="before">I </span>章节一',
    );
    expect(result).toContain(
      '<span data-ahafair-counter-generated="before">II </span>章节二',
    );
    expect(result).toContain(
      '<span data-ahafair-counter-generated="before">a-a </span>条目A',
    );
    expect(result).toContain(
      '<span data-ahafair-counter-generated="before">a-b </span>条目B',
    );
